...
function Home({ language, path, chapters, navigate, openMenu }: { language: Language; path: string; chapters: Chapter[]; navigate: Navigate; openMenu: () => void }) {
  ...
  return <div className={`site-shell language-${language}`}>
    ...
    <main>
      <section className="hero"><div className="hero-copy"><p className="eyebrow">{text.eyebrow}</p><h1>{text.heroLead}<br /><em>{text.heroEm}</em></h1><p className="dek">{text.dek}</p><div className="hero-actions"><ReaderLink className="primary-action" href={localizedPath(language, `/read/${startChapter.slug}`)} navigate={navigate}>{lastSlug ? text.continueReading : text.startReading} <span>→</span></ReaderLink><span className="reading-time">{text.bookStats}</span></div></div><figure className="cover-card"><img src="/images/cover.png" alt={text.coverAlt} /></figure></section>
